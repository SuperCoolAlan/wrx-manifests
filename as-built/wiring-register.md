...
| | | | | Cut is on the BY run between R1 pin 7 and R122 pin 10. Whether it passes through an R166/R167 pair depends on an unresolved question - see WHICH FUEL PUMP DIAGRAM APPLIES. Either way the cut point and both endpoints are unchanged. Splice method not yet recorded | |
| ✅ | Fuel pump - factory control chain DEAD | ECM conn D (B137) pin 15 -> ... -> R122 · D15 · GR at the ECM, BY from the relay onward | **abandoned** | Factory chain: ECM connector D pin 15 (GR) commands the FUEL PUMP RELAY; the relay output (BY) runs B46 -> B97/R1 bulkhead (8-pin, area B-2) -> R122 pin 10 at the fuel pump controller. On STi-model cars an R166/R167 pair sits between R1 and R122 - see the diagram question | Superseded by the hardwired relay |
| | | | | Chain is INTACT up to R1 pin 7. Past the cut it feeds the new relay coil. R166/R167 and R122 carry nothing | |
| ⚠️ | WHICH FUEL PUMP DIAGRAM APPLIES | — | **reference** | This circuit is BODY-SIDE wiring, and the body harness is 2004 WRX - so E/G(TB)-01 (WI-82, turbo) should govern, NOT the STi E/G(ST)-01 (WI-108). R166/R167 are indexed "(STi model)", meaning the STi CAR, not the STi engine. The 2005 STi ENGINE harness does not reach this circuit | 2004 WRX body harness retained; only the engine harness is STi |
| | | | | OPEN: Alan traced the run as passing through R166/R167, which a 2004 WRX body harness should not have. PHYSICALLY CHECK for an extra 6-pin connector pair between R1 and R122. If it is absent, the turbo diagram is correct and the cut is R1 pin 7 straight to R122 pin 10 | |
...
- [ ] **WHICH FUEL PUMP DIAGRAM APPLIES** — OPEN: Alan traced the run as passing through R166/R167, which a 2004 WRX body harness should not have. PHYSICALLY CHECK for an extra 6-pin connector pair between R1 and R122. If it is absent, the turbo diagram is correct and the cut is R1 pin 7 straight to R122 pin 10
