...
chassis_hole_tilt    = 30.0;  // [MEASURE sign] bolt axis rises this far above
                              // horizontal, nose forward. Negate if it noses DOWN.
tower_boss_w         = 26.0;  // [MEASURE] boss width along X
tower_boss_h         = 18.0;  // [MEASURE] boss height across the bolt axis
tower_boss_r         = 3.0;
tower_boss_clear     = 1.5;   // air around each boss where the hook wraps past it

// ---- MOUNT HOOK -----------------------------------------------------------
// Narrowed from v1 on purpose. The seat pads and arms are keep-out volumes for
// the FPR's can and adjuster: at 42mm pads / 26mm arms / tie rib only 3 FPR
// positions cleared them, at 28/18/none it is 15. Widening these again will
// re-strangle the layout, so do not grow them without re-running the checks.
mount_pad_t      = 6.0;   // seat pad thickness along the bolt axis
mount_pad_w      = 28.0;  // pad width along X, per bolt
mount_pad_h      = 28.0;  // pad height across the axis, in the tilted plane
mount_arm_w      = 18.0;  // hook arm width along X
mount_arm_root_h = 24.0;  // how far down the panel each arm blends in
mount_arm_root_dx= 16.0;  // root swept OUTBOARD of its bolt by this much. The FPR's
                          // steel ear plate lands flat on the panel across X 5..75;
                          // a root sitting square under the bolt collides with it.
mount_tie        = false; // tie rib across the pads' top edge. OFF: at any useful
                          // FPR height the adjuster knob comes up through it, and
                          // you want that knob reachable anyway.
mount_tie_h      = 12.0;
...
panel_left_ext  = 0.0;    // back to the measured 137mm working width. NOTE: the
                          // components are deliberately left where they are, so the
                          // cradle now overhangs this edge — see the report.
drop_left       = 170.0;  // panel depth below Z=0 at panel_left_meas
drop_right      = 132.0;  // ... and at panel_right_x
...
fpr_ear_hole_d  = 5.5;
fpr_x           = 40.0;   // ear-pair centre X. Sits BETWEEN the two hook arms so
                          // the can rises through the gap between them.
fpr_ear_z       = -33.0;  // ear-hole centreline Z. Raised from v1's -36: with the
                          // tie rib gone the can is free to come up, and every mm
                          // it rises is a mm freed underneath for the sensor.
                          // Above ~-30 the ear hole runs out of panel at the top.
...
ffs_ox  =  48.0;  // cradle-local origin, bracket X
ffs_oz  = -112.0; // cradle-local origin, bracket Z
ffs_rot = -45.0;  // in-plane rotation. Passage angle off horizontal = 90-|rot|.
ffs_hole_d = 6.5; // cradle ear holes (M5 rattles 1mm — washer under the head)

// Vapour-lock guard. Do not relax this below ~30 without a reason.
ffs_passage_deg = 90 - abs(ffs_rot);
assert(ffs_passage_deg >= 30,
...
side_fitting_dir_r   = -1;    // which way the return leg lies: +1 up, -1 down.
                              // -1: the U loops DOWN and back to the left.
...
return_fitting_len   = 50;    // [MEASURE]
return_fitting_ang   = 45;    // degrees off straight-down: 0 down, 90 level, >90 rising
return_fitting_clock = -90;   // which way the bend swings, about the port axis.
                              // 0 = rearward (-Y), which buries the hose in the back
                              // panel — there is no gap behind it. -90 = toward -X:
                              // the line leaves down and to the RIGHT, clear of the
                              // sensor body, to be joined to one of its side ports.
gauge_stub_dia       = 16;
gauge_stub_len       = 18;
vac_nipple_dia       = 8;
vac_nipple_len       = 25;
...
module chassis_bosses() { mount_pads(); if (mount_tie) mount_tie_rib(); mount_arms(); }

module mount_pads() {
    for (cx = [cx0, cx1])
        mount_frame() translate([cx, 0, 0])
            linear_extrude(mount_pad_t)
                offset(r = 5) offset(r = -5)
                    square([mount_pad_w, mount_pad_h], center = true);
}

module mount_tie_rib() {
    mount_frame() translate([cx0, -mount_pad_h/2 + mount_tie_h/2, 0])
        linear_extrude(mount_pad_t)
            square([chassis_hole_spacing, mount_tie_h], center = true);
}

module mount_arms() {
    for (cx = [cx0, cx1])
        hull() {
            mount_frame() translate([cx - mount_arm_w/2, -mount_pad_h/2, 0])
                cube([mount_arm_w, mount_pad_h, mount_pad_t]);
            translate([cx + (cx > cx0 ? mount_arm_root_dx : -mount_arm_root_dx)
                          - mount_arm_w/2, 0, panel_top_z - mount_arm_root_h])
                cube([mount_arm_w, panel_t, mount_arm_root_h]);
        }
}
...
module ghosts() { tower_boss_ghost(); fpr_solid(); ffs_solid(); }
...
module fpr_return_fitting() {
    translate([fpr_x, panel_t + axis_from_plane, fpr_base_z])
        rotate([0, 0, return_fitting_clock]) rotate([180 - return_fitting_ang, 0, 0])
...
module fpr_solid(show_return = true) {
    ...
    translate([fpr_x, panel_t + axis_from_plane, 0]) {
        ...
        translate([-block_w/2, 0, bz + port_axis_above_base]) {
            ...
            translate([0, 0, side_fitting_dir_r * side_fitting_off_r]) rotate([0, -90, 0])
                cylinder(d = side_fitting_dia, h = side_fitting_len_r, $fn = 32);
            hull() for (dz = [0, side_fitting_dir_r * side_fitting_off_r])
                translate([-side_fitting_len_r, 0, dz])
